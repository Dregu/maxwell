...
template <size_t N>
constexpr auto find_first_of_start(const std::array<char, N> &data,
                                   size_t start, char ch) {
  size_t idx = start;
...
template <size_t N>
constexpr auto find_first_not_of_start(const std::array<char, N> &data,
                                       size_t start, char ch) {
...
template <typename T, T F = 16> constexpr T concat_hex(T lhs, T rhs) {
  return F * lhs + rhs;
}
} // namespace detail

template <auto R> struct compiletime_value {
  using type = decltype(R);
  constexpr static type value = R;
};

//	string wrapper
//	potential todo: make interface class that includes detail utilities at
//shorthand?
template <size_t N> struct compiletime_string_wrapper {
...
template <size_t N>
compiletime_string_wrapper(const char (&)[N])
    -> compiletime_string_wrapper<N - 1>;

template <char delimiter, char mask, int masked>
struct basic_compiletime_string_to_byte_array_data {
  template <compiletime_string_wrapper str> struct getter {
  private:
    struct parse {
      struct result {
      ...
      constexpr static auto get() {
        ...
        constexpr size_t start =
            detail::find_first_not_of_start(str(), 0, delimiter);
        constexpr size_t next =
            detail::find_first_of_start(str(), start, delimiter);
      ...
      for (auto i = next; i < end; ++i) {
        for (auto entry : skips) {
          if (entry == i && entry < end) {
            size_t idx =
                detail::find_first_not_of_start(str(), i + 1, delimiter);
...
using compiletime_string_to_byte_array_data =
    basic_compiletime_string_to_byte_array_data<' ', '?', -1>;
} // namespace stb
